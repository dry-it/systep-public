...
export class DataService {
  ...
  get(path: string) {
    return this.http.get(this.baseurl + path, { responseType: 'blob' as 'json', })
  }
  ...
  test(path: string, body: any) {
    ...
    this.auth.currentUser
      .then(user => {
        user.getIdToken()
          .then((token) => {
            this.http.post(this.baseurl + path, body, { headers: new HttpHeaders().set('Authorization', 'Bearer ' + token) })
